...
/**
* A sequence of Distance instances.
*/
public final class DistanceSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DistanceSeq (int initialMaximum) {
        super(Distance.class, initialMaximum);
    }

    public DistanceSeq (Collection<?> elements) {
        super(Distance.class, elements);
    }

    public  Distance get(int index) { 
        return (Distance) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, Distance.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' Distance objects (beyond the original size of this object)
