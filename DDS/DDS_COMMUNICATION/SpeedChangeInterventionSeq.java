...
/**
* A sequence of SpeedChangeIntervention instances.
*/
public final class SpeedChangeInterventionSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SpeedChangeInterventionSeq() {
        super(SpeedChangeIntervention.class);
    }

    public SpeedChangeInterventionSeq (int initialMaximum) {
        super(SpeedChangeIntervention.class, initialMaximum);
    }

    public SpeedChangeInterventionSeq (Collection<?> elements) {
        super(SpeedChangeIntervention.class, elements);
    }

    public  SpeedChangeIntervention get(int index) { 
        return (SpeedChangeIntervention) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SpeedChangeIntervention.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SpeedChangeIntervention objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SpeedChangeIntervention.create());
